...
export async function fetchInspectionForPdf(inspectionId: string): Promise<InspectionPdfData | null> {
  // Pattern: ensure we only query when auth session exists.
  const {
    data: { session },
    error: sessionError,
